...
export default function Reports() {
    ...
    const fetchSettings = async () => {
        try {
            ...
            const res = await axios.get('/api/exit/settings', { headers: { Authorization: `Bearer ${token}` } });
    ...
    const fetchDaily = async () => {
        try {
            const token = localStorage.getItem('token');
            const res = await axios.get('/api/exit/permissions', {
    ...
    const searchStudent = async (e: React.FormEvent) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const res = await axios.get(`/api/exit/student/${studentId}/history`, {
                headers: { Authorization: `Bearer ${token}` }
            });
