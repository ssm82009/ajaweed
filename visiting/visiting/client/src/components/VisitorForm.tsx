...
const VisitorForm: React.FC = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            navigate('/success', { state: { serial: res.data.serial_number } });
        } catch (err: any) {
        ...
        <div className="card">
            ...
            {step === 7 && (
                <div>
                    ...
                    <div style={{ textAlign: 'right', marginBottom: '1rem', background: '#1e293b', padding: '1rem', borderRadius: '8px', fontSize: '0.9rem' }}>
                        <p style={{ margin: '5px 0' }}><strong>الاسم:</strong> {formData.visitor_name}</p>
                        <p style={{ margin: '5px 0' }}><strong>الهوية:</strong> {formData.id_number}</p>
                        <p style={{ margin: '5px 0' }}><strong>سبب الزيارة:</strong> {formData.reason}</p>
                    </div>
                    ...
                    <div className="input-group">
                        ...
                        <div className="signature-pad">
                            <SignatureCanvas
                                ref={sigPad}
                                penColor="black"
                                canvasProps={{ width: 350, height: 160, className: 'sigCanvas' }}
                            />
                        </div>
            ...
            <div style={{ marginTop: '2rem', textAlign: 'center', opacity: 0.3 }}>
                <small onClick={() => navigate('/exit')}>تسجيل خروج (QR Scan)</small>
            </div>
        </div>
    );
};
