...
const AdminDashboard: React.FC = () => {
    ...
    const loadVisits = async () => {
        try {
            const res = await axios.get(`http://localhost:3001/api/admin/visits?date=${dateFilter}`);
            setVisits(res.data);
    ...
    const saveSettings = async () => {
        try {
            await axios.post('http://localhost:3001/api/admin/settings', settings);
    ...
    return (
        <div className="dashboard-container">
            <div className="no-print" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                ...
                <button className="secondary-btn" onClick={() => navigate('/')} style={{ margin: 0 }}>خروج</button>
            </div>
